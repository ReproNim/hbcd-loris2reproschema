...
def get_value_type(validation_type: str) -> str:
    ...
    if "integer" in validation_type:
        return "xsd:integer"
    elif "float" in validation_type or "number" in validation_type or "decimal" in validation_type:
        return "xsd:float"
...
class ReproSchemaConverter:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.column_mappings = config.get("column_mappings", DEFAULT_LORIS_TO_REPROSCHEMA_MAP)
        
        # Additional notes fields to include (can be customized in config)
        self.additional_notes_list = config.get("additional_notes_fields", ADDITIONAL_NOTES_LIST)
    ...
    def analyze_csv_headers(self, df: pd.DataFrame) -> Dict[str, List[str]]:
        ...
        domain_cols = [col for col in columns if col.lower() in 
                      ['domain', 'instrument', 'form', 'full_instrument_name', 'category', 'assessment']]
        if domain_cols:
            suggestions['domain_column'] = domain_cols[0]
            self.log(f"Suggested domain column: {domain_cols[0]}")
        
        # Look for item name column
        name_cols = [col for col in columns if col.lower() in 
                    ['name', 'item', 'variable', 'field', 'fieldname']]
        if name_cols:
            suggestions['item_name'] = name_cols[0]
            self.log(f"Suggested item name column: {name_cols[0]}")
        
        # Look for question text column
        question_cols = [col for col in columns if col.lower() in 
                        ['question', 'description', 'label', 'text']]
        if question_cols:
    ...
    def load_csv(self, csv_file: str, encoding: Optional[str] = None) -> pd.DataFrame:
        """
        ...
        try:
            ...
            if encoding:
                ...
                for enc in encodings:
                    try:
                        ...
                        self.log(f"Failed to decode with {enc}, trying next encoding...", "DEBUG")
            ...
            required_cols = [self.column_mappings[col] for col in ["item_name"]]
            missing_cols = [col for col in required_cols if col not in df.columns]
            if missing_cols:
                self.log(f"Warning: Required columns missing: {', '.join(missing_cols)}", "WARNING")
    ...
    def preprocess_fields(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if domain_col not in df.columns:
            ...
            self.log(f"Warning: Domain column '{domain_col}' not found. Creating it with value '{default_domain}'", "WARNING")
            df[domain_col] = default_domain
        
        return df

    def process_input_value_types(self, field_type: str, data_type: str) -> Tuple[str, str, Optional[Dict]]:
        """
    ...
    def process_response_options(self, row: Dict[str, Any], item_name: str) -> Tuple[Dict[str, Any], Optional[Dict]]:
        """
        ...
        input_type, value_type, type_notes = self.process_input_value_types(field_type, data_type)
        ...
        is_multiple_choice = "multi" in field_type.lower() or "checkbox" in field_type.lower()
        ...
        if (labels_col in row and values_col in row and 
            row.get(labels_col) and row.get(values_col)):
        ...
        if min_val_col in row and row.get(min_val_col):
            try:
                ...
                response_options["minValue"] = float(min_val) if '.' in str(min_val) else int(min_val)
            except (ValueError, TypeError) as e:
                self.log(f"Warning: Invalid min value for {item_name}: {row.get(min_val_col)}", "WARNING")
                
        if max_val_col in row and row.get(max_val_col):
            try:
                max_val = row.get(max_val_col)
                response_options["maxValue"] = float(max_val) if '.' in str(max_val) else int(max_val)
            except (ValueError, TypeError) as e:
                self.log(f"Warning: Invalid max value for {item_name}: {row.get(max_val_col)}", "WARNING")
                
        return response_options, additional_notes

    def process_choices(self, labels_str: str, values_str: str, item_name: str) -> Tuple[List[Dict[str, Any]], List[str]]:
        """
        ...
        """
        if pd.isna(labels_str) or pd.isna(values_str) or not labels_str or not values_str:
            return [], ["xsd:string"]
        ...
        try:
            ...
            if ((str(labels_str).startswith('[') and str(labels_str).endswith(']')) or
                (str(values_str).startswith('[') and str(values_str).endswith(']'))):
                try:
                    ...
                    if not isinstance(labels, list) or not isinstance(values, list):
                        raise ValueError("Parsed labels or values are not lists")
                    if len(labels) != len(values):
                        self.log(f"Warning: Mismatch in number of labels and values for {item_name} after ast.literal_eval. Labels: {labels_str}, Values: {values_str}", "WARNING")
                        return [], ["xsd:string"]

                except (ValueError, SyntaxError) as e_ast:
                    self.log(f"ast.literal_eval failed for {item_name}: {str(e_ast)}, trying json.loads", "DEBUG")
                    try:
                        import json
                        ...
                        if len(labels) != len(values):
                            self.log(f"Warning: Mismatch in number of labels and values for {item_name} after json.loads. Labels: {labels_str}, Values: {values_str}", "WARNING")
                            return [], ["xsd:string"]
                    except Exception as e_json:
                        # If JSON parsing fails, fall back to string splitting
                        self.log(f"JSON parsing failed for {item_name}: {str(e_json)}, falling back to split", "DEBUG")
                        # Remove brackets and split by commas
                        labels = [l.strip() for l in str(labels_str).strip('[]').split(',')]
                        values = [v.strip() for v in str(values_str).strip('[]').split(',')]
                        if len(labels) != len(values):
                            self.log(f"Warning: Mismatch in number of labels and values for {item_name} after string split. Labels: {labels_str}, Values: {values_str}", "WARNING")
                            return [], ["xsd:string"]
            ...
            self.log(f"Warning: Error processing response options for {item_name}: {str(e)}", "WARNING")
    ...
    def process_preamble(self, row: Dict[str, Any], prior_preamble_info: Optional[Dict] = None) -> Tuple[Dict[str, str], Optional[Dict]]:
        """
        ...
        if visibility_col and visibility_col in row and row[visibility_col]:
            ...
            if condition:
                ...
                return preamble_text, None  # Don't propagate this type of preamble
        
        # If no preamble found, return None
        return None, None

    def process_item(self, row: Dict[str, Any], prior_preamble_info: Optional[Dict] = None) -> Tuple[Dict[str, Any], Optional[Dict], Optional[Dict], Dict[str, Any]]:
        """
        ...
        if not item_name or pd.isna(item_name):
            ...
            if "source_field" in row and row["source_field"] and not pd.isna(row["source_field"]):
        ...
        if description and not pd.isna(description) and parse_html(description) != question_text:
            item_data["description"] = parse_html(description)

        # Process preamble (conditional logic)
        preamble, preamble_info_propagate = self.process_preamble(row, prior_preamble_info)
        if preamble:
        ...
        if annotation_col and annotation_col in row and row[annotation_col]:
            ...
            if "@READONLY" in annotation or "@HIDDEN" in annotation or "@CALCTEXT" in annotation:
                item_data["ui"]["readonlyValue"] = True
        ...
        if required and not pd.isna(required):
            ...
            elif str(required).lower() in ('y', 'yes', 'true', '1', 'required', 'optional'):
                is_required = str(required).lower() in ('y', 'yes', 'true', '1', 'required')
    ...
    def process_dataframe(self, df: pd.DataFrame) -> Dict[str, Dict[str, Any]]:
        ...
        fallback_domain_col = self.config.get("fallback_domain_column", "domain")
        default_domain = self.config.get("default_domain", "Unknown")
        
        # Ensure we have some domain column
        if primary_domain_col not in df.columns and fallback_domain_col not in df.columns:
            self.log(f"Warning: Neither primary domain column '{primary_domain_col}' nor fallback '{fallback_domain_col}' found. Using default '{default_domain}'", "WARNING")
            df["temp_domain"] = default_domain
            domain_col = "temp_domain"
        elif primary_domain_col not in df.columns:
            self.log(f"Warning: Primary domain column '{primary_domain_col}' not found. Using fallback '{fallback_domain_col}'", "WARNING")
        ...
        if fallback_domain_col in df.columns and primary_domain_col in df.columns:
            mask = df[primary_domain_col].isna() | (df[primary_domain_col] == "")
            if fallback_domain_col in df.columns:  # Double-check to avoid KeyError
                df.loc[mask, "effective_domain"] = df.loc[mask, fallback_domain_col]
        ...
        self.log(f"Found {num_domains} unique domains/activities after applying fallbacks")
        ...
        for domain, group in domain_groups:
            ...
            self.log(f"Processing domain: {domain_str} with {item_count} items")
            ...
            for _, row in group.iterrows():
                try:
                    item, item_preamble_info, compute, addProperty = self.process_item(
                    ...
                    self.log(f"Error processing item in domain {domain_str}: {str(e)}", "ERROR")
                    import traceback
            ...
            if (act_preamble and len(set(act_preamble)) == 1 and 
                len(act_preamble) == len(act_items_order)):
        ...
        self.log(f"Successfully processed {total_items} items into {len(activities)} activities")
        return activities
    ...
    def create_activity_schema(
        ...
        self.log(f"{activity_name} activity schema created with {len(activity_data['items'])} items")
    ...
    def create_protocol_schema(
        ...
        protocol_schema = {
            ...
            "prefLabel": {"en": protocol_data.get("protocol_display_name", protocol_name)},
            "description": {"en": protocol_data.get("protocol_description", f"Protocol for {protocol_name}")},
            "schemaVersion": get_context_version(self.schema_context_url),
            ...
            for item in output_path.iterdir():
                if item.is_dir():
                    try:
                        ...
                        self.log(f"Warning: Could not remove directory {item}: {str(e)}", "WARNING")
            ...
            for file in files:
                if file == ".DS_Store":
                    try:
                        ...
                        self.log(f"Warning: Could not remove file {file_path}: {str(e)}", "WARNING")

    def analyze_csv(self, csv_file: str, encoding: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            domain_suggestions = [col for col in columns if col.lower() in 
                                 ['domain', 'instrument', 'form', 'category', 'assessment', 'full_instrument_name']]
            
            name_suggestions = [col for col in columns if col.lower() in 
                               ['name', 'item', 'variable', 'field', 'fieldname']]
            
            question_suggestions = [col for col in columns if col.lower() in 
                                   ['question', 'description', 'text', 'label']]
            
            # Look for response options
            option_label_suggestions = [col for col in columns if 'label' in col.lower() or 'option' in col.lower()]
            option_value_suggestions = [col for col in columns if 'value' in col.lower() or 'option' in col.lower()]
            
            # Look for field type columns
            field_type_suggestions = [col for col in columns if 'type' in col.lower() or 'field' in col.lower()]
            ...
            if option_label_suggestions:
                suggested_mappings["response_option_labels"] = option_label_suggestions[0]
                
            if option_value_suggestions:
                suggested_mappings["response_option_values"] = option_value_suggestions[0]
    ...
    def convert(self, csv_file: str, output_path: str, encoding: Optional[str] = None) -> None:
        """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Convert LORIS CSV data to ReproSchema format."
    )
    ...
    try:
        ...
        if args.analyze:
            ...
            analysis = converter.analyze_csv(args.csv_file, args.encoding)
        ...
        # Run conversion
        converter.convert(args.csv_file, args.output_path, args.encoding)
